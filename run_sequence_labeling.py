...
def main(args):
    ...
    if args.do_train:
        ...
        warmup_steps = max(int(max_train_steps * args.warmup_proportion),1)
        ...
        while True:
            try:
                ...
                if steps % args.skip_steps != 0:
                    ...
                    fetch_list = [graph_vars[i].name for i in fetch_list_name]
                    outputs = train_exe.run(fetch_list=fetch_list)
                    if steps % (args.skip_steps*50+1) == 0:
                        for i, item in enumerate(fetch_list_name):
                            print(item )
                            print(outputs[i])
                            print('\n')
                else:
                ...
                if steps % valid_step == 0:
                    ...
                    if args.do_test:
                        ...
                        if outputs["f1"] > best_f1 and outputs["f1"] > args.min_f1:
                            ...
                            fluid.io.save_persistables(exe, save_path, train_program)
                            print('save test model at step %d, f1 score %f' % (steps, outputs['f1']))
